...
{isuperarray=1,isuperarraymax=(SUPERARRAYNAME//Length),userdefined={},
...
(*special initial value for this superarray*)
...
IMAGEPADDING->{{0, 20}, {0,20}},
PLOTRANGEPADDING->Scaled[.1],
PLOTMARKERS->{Automatic, 0.015},
PLOTRANGE->Full,
LABELSTYLE->Directive[Black,Bold,16]
};

PLOTLEGENDS=Placed[Alphabet[]//ToUpperCase//Table["expert "<>#[[i]],{i,1,#//Length}]&,Above];
...
meanrewards=Mean/@(CREWARD[[iexperiment]]//Transpose);
plot1=(CREWARD[[iexperiment]]//ListPlot[#,AspectRatio->ASPECTRATIO,ImageSize->IMAGESIZE,ImagePadding->IMAGEPADDING,PlotRangePadding->PLOTRANGEPADDING,PlotMarkers->PLOTMARKERS,PlotRange->PLOTRANGE,AxesLabel->{"j","\!\(\*SubscriptBox[\(r\), \(ij\)]\)(cumul.)"},LabelStyle->LABELSTYLE,PlotLegends->PLOTLEGENDS]&/.PLOTSTYLE3);

plot2=(meanrewards//ListPlot[#,AspectRatio->ASPECTRATIO,ImageSize->IMAGESIZE,ImagePadding->IMAGEPADDING,PlotRangePadding->PLOTRANGEPADDING,PlotMarkers->marker2[],PlotRange->PLOTRANGE,AxesLabel->{"j","\!\(\*SubscriptBox[\(r\), \(ij\)]\)(cumul.)"},LabelStyle->LABELSTYLE,PlotLegends->Placed[PointLegend[{Black},{"Mean"}],Above]]&/.PLOTSTYLE3);
...
Which[
currentpos==1, AppendTo[cumulrewards,experiment[[-1,INDEX["r"]]]],
currentpos>1,AppendTo[cumulrewards,experiment[[-1,INDEX["r"]]]+cumulrewards[[-1]]],
True,Nothing
];
Unprotect[CREWARD];
...
{currentpos=experiment//Length,cumulsurprisal=CSURPRISAL[[-1]]//Transpose,avgsurprisalj=ASURPRISAL[[-1]]//Transpose},
(*Print["currentpos=",currentpos];
Print["CSURPRISAL[[-1]]=",cumulsurprisal];
Print["tobe appended: ",experiment[[-1,INDEX["s"]]]];*)
Which[(*cumulative surprisal*)
currentpos==1, AppendTo[cumulsurprisal,experiment[[-1,INDEX["s"]]]],
currentpos>1,AppendTo[cumulsurprisal,experiment[[-1,INDEX["s"]]]+cumulsurprisal[[-1]]],
True,Nothing
];
Unprotect[CSURPRISAL];
CSURPRISAL[[-1]]=cumulsurprisal//Transpose;
Protect[CSURPRISAL];

Which[(*avgj surprisal*)
currentpos>=1, AppendTo[avgsurprisalj,cumulsurprisal[[-1]]/currentpos],
True,Nothing
];
Unprotect[ASURPRISAL];
ASURPRISAL[[-1]]=avgsurprisalj//Transpose;
Protect[ASURPRISAL];
]
...
(*Print["asking question:",CREWARD];*)
askquestion[experiment];
(*Print["calculating cumul rewards",CREWARD];*)
calcstatrewards[experiment];
(*Print["calculating cumul surprisal",CREWARD];*)
